...
namespace DataTableAccessLayer
{
    ...
    public class SqlDataTableAccessLayer
    {
        ...
        public void LoadDataTable(DataTable table, string query)
        {
            table.Clear();
        ...
        public string GetTotalFromSales(string query)
        {
            decimal stock = 0;
            var sqlCommand = new SqlCommand(query, sqlConnection);
            Debug.WriteLine("LoadDataTable: " + sqlCommand.CommandText);
            ...
            using (SqlDataReader dataReader = sqlCommand.ExecuteReader())
            {
                ...
                {
                    dataReader.Read();
                    stock += Convert.ToDecimal(dataReader["total"] is DBNull ? 0 : dataReader["total"]);
                }
            }
            return stock+"";
        }
